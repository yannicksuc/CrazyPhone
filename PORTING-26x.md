...
| `AbstractContainerScreen#renderBackground(GuiGraphics,...)` (dim overlay) **+** `#renderBg(GuiGraphics, float, int, int)` (screen's own texture), two separate hooks | **Merged into one**: `Screen#extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick)` - note `partialTick` moved from 2nd position to *last*. A screen now overrides this ONE method to do both the dim fill and its own texture (see vanilla's `InventoryScreen#extractBackground`, which calls `super.extractBackground(...)` then blits its own texture, in one method). | **Real structural merge, not a rename.** `CrazyPhoneDefaultScreenScreen.java`'s current `renderBackground()` + `renderBg()` pair needs to become one `extractBackground()` override. Every subclass that currently overrides `renderBg()` on top of the base class needs the same rethink - **not yet investigated per-subclass**, and this is exactly the kind of thing likely to have more surprises once actually attempted (e.g. does the base class need to expose a new protected hook of its own for subclasses to still layer onto, mirroring what `renderBg()` used to let them do?). |
