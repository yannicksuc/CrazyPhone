...
@EventBusSubscriber
public class PhoneGiveCommand {
	@SubscribeEvent
	public static void registerCommand(RegisterCommandsEvent event) {
		event.getDispatcher().register(Commands.literal("phoneGive").requires(s -> s.hasPermission(4)).then(Commands.argument("number", StringArgumentType.word()).executes(arguments -> {
