...
//?}
    ...
    *///? } else {
    ...
    static void handle(ServerPlayer player, UUID photoId) {
        ...
        if (entry == null) {
            org.slf4j.LoggerFactory.getLogger("crazyphone").warn("give photo item refused: photo {} is not stored on the server", photoId);
            return;
        }
        ...
        if (!authorized) {
            org.slf4j.LoggerFactory.getLogger("crazyphone").warn("give photo item refused for {}: requester number=[{}] (empty = phone not in main hand), photo owner=[{}], conversation=[{}]",
